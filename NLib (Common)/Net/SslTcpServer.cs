    ...
    public class SslTcpServer
    {
        ...
        public void StartListening()
        {
            ...
            while (true)
            {
                ...
                SimpleThread.BeginInvoke(new SimpleThreadMethod(
                () =>
                {
                    var e = new ClientConnectionEventArgs(client);
                    e.SslStream.AuthenticateAsServer(_certificate, false, SslProtocols.Tls, true);
                    e.SslStream.ReadTimeout = _millisecondsTimeout;
                    e.SslStream.WriteTimeout = _millisecondsTimeout;
                    OnClientConnected(new ClientConnectionEventArgs(client));
                }));
            }
        }
